...
\subsection{Intelligente Betriebsautomatisierung und Orchestrierung}
\label{subsec:intelligente-betriebsautomatisierung}
Das Muster Intelligente Betriebsautomatisierung und Orchestrierung beschreibt den Einsatz von \gls{ki} im laufenden Plattformbetrieb.
Der Schwerpunkt liegt auf der automatisierten Steuerung von Systemzuständen, um Stabilität und Verfügbarkeit der Plattform sicherzustellen.
...
Verarbeitet werden Metriken, Logs und Zustandsinformationen aus dem Cluster (z. B. \gls{cpu}-Auslastung), die aus dem laufenden Betrieb anfallen.
Anhand historischer Verläufe werden typische Lastmuster und Abweichungen vom Normalbetrieb erkannt.
Darauf aufbauend werden operative Maßnahmen abgeleitet, die automatisiert ausgeführt werden können.
Prädiktives Auto-Scaling stellt einen Teilaspekt des Musters dar.
Der Fokus liegt auf der Orchestrierung im Betrieb, bei der Lastverteilung und Reaktionen auf bekannte Fehlerbilder koordiniert werden \cite{zaaloukCLOUDNATIVEARTIFICIAL,jossonpaulkalapparambathAdvancingDistributedSystems2025}.
Der Nutzen zeigt sich in einer Reduktion manueller Betriebseingriffe sowie in stabileren Laufzeiteigenschaften der Plattform.
...
Die Umsetzung erfordert eine durchgängige Erfassung von Betriebsdaten sowie automatisierbare Eingriffsmöglichkeiten in die Plattform.
