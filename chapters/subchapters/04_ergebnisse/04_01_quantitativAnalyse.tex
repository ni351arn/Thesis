...
Im Jahr 2024 wurden sieben und 2025 wurden acht Publikationen identifiziert, überwiegend Journalartikel, ergänzt durch jeweils einen Konferenzbeitrag, ein ArXiv-Paper und ein Whitepaper. 
Dies weist auf ein zunehmendes wissenschaftliches Interesse am Einsatz von \gls{ki} in Cloud-Native- und Platform Engineering-Kontexten hin.
Der zeitliche Anstieg der Publikationen fällt mit den jüngsten Fortschritten im Bereich generativer \gls{ki} zusammen \cite{luComputingEraLarge2024a}.
...
Optimierung von \gls{ci}/\gls{cd}-Pipelines, Ressourcen- und Workload-Optimierung, Sicherheits- und Bedrohungserkennung sowie Betrieb und Orchestrierung der Plattform.
Die vier Anwendungsbereiche wurden auf Basis der in den Studien jeweils dominanten Einsatzschwerpunkte gebildet.
...
Dies deutet darauf hin, dass \gls{ki}-Ansätze besonders im laufenden Betrieb relevant sind, etwa zur Unterstützung von Monitoring oder der automatisierten Steuerung von Plattformkomponenten \cite{reddygopireddyIntegratingAIDevOps2022}.
Die Optimierung von \gls{ci}/\gls{cd}-Pipelines und Sicherheits- und Bedrohungserkennung sind mit jeweils drei Publikationen vertreten.
Beide Anwendungsfelder werden in vielen Arbeiten aufgegriffen, stehen jedoch seltener im Fokus der jeweiligen Studie.
Die Optimierung der \gls{ci}/\gls{cd}-Pipeline wird oft als Teilaspekt in ein größeres Gesamtbild der Infrastruktur-Transformation behandelt \cite{supritpattanayakIntegratingAIDevOps2024}.
...
In der Praxis überschneiden sich die Anwendungsbereiche häufig, da \gls{ki}-Lösungen oft mehrere Aufgaben gleichzeitig unterstützen.
...
Im nächsten Schritt wurden die in den Publikationen beschriebenen Herausforderungen analysiert, die beim Einsatz von \gls{ki} im Platform Engineering auftreten. 
...
Ebenfalls häufig werden Skalierbarkeit, Latenz und Monitoring mit 83 \% (15/18) sowie \gls{ki}-Governance, Datenschutz und Compliance mit 83 \% (15/18) adressiert.
Integrationskomplexität und Abhängigkeiten werden in 78 \% (14/18) der Studien thematisiert.
...
Die Ergebnisse zeigen ein Spannungsfeld.
\gls{ki} wird eingesetzt, um Plattformen effizienter und kostengünstiger zu betreiben.
Gleichzeitig verursachen Training, Inferenz sowie zusätzliche Überwachungs- und Datenpipelines selbst spürbare Ressourcen- und Betriebskosten \cite{zaaloukCLOUDNATIVEARTIFICIAL}.
Die hohe Nennung von \gls{ki}-Governance, Datenschutz und Compliance legt zudem nahe, dass der produktive \gls{ki}-Einsatz in Plattformen häufig weniger an der reinen technischen Machbarkeit scheitert. 
Vielmehr wird er stark durch Anforderungen an Datenzugriff, Nachvollziehbarkeit und Regelkonformität begrenzt \cite{tamanampudiAIEnhancedContinuousIntegration}.
\begin{figure}[H]
    \centering
...
Ein weiterer Bestandteil der quantitativen Analyse umfasst die in den Publikationen verwendeten Lernparadigmen von \gls{ml}. 
...
Die Auswertung zeigt, dass \gls{sl} in den meisten Arbeiten eine zentrale Rolle spielt. 
Es wird in vier Publikationen ausdrücklich beschrieben, elfmal implizit erkennbar und in zwei kurz erwähnt. 
\gls{rl} wird insgesamt siebenmal explizit genannt und in vier weiteren Arbeiten erwähnt. 
\gls{ul} tritt mit drei expliziten und sieben impliziten Nennungen seltener auf, ist jedoch ebenfalls präsent.
...
Auffällig ist die deutliche Differenz zwischen explizit und implizit erkennbaren Anwendungen, insbesondere bei \gls{sl} und \gls{ul}. 
...
Ein Erklärungsansatz für die Dominanz von \gls{sl} ist, dass in Plattformumgebungen häufig gut messbare Zielgrößen und bereits beschriftete Verlaufsdaten vorliegen (z. B. Störungsmeldungen, Support-Tickets oder Metriken mit bekanntem Ergebnis), wodurch sich \gls{sl}-Ansätze besonders gut anwenden und bewerten lassen \cite{tamanampudiAIEnhancedContinuousIntegration}.
...
Die Analyse zeigt, dass im Kontext des Platform Engineerings eine Vielfalt an \gls{ki}-Verfahren und \gls{ml}-Methoden eingesetzt wird.
Für eine systematische Bewertung wurde eine eigene Kategorisierung entwickelt.
Den Ausgangspunkt bildet die Tabelle aus \textcite{enemosahEnhancingDevOpsEfficiency2025}, in der verschiedene \gls{ki}-Techniken für Testfallpriorisierung beschrieben werden. 
...
Die Ergebnisse zeigen ein deutliches Übergewicht von \gls{dl} / \gls{nn}, die in 89 \% (16/18) der Publikationen eingesetzt oder thematisiert werden. 
...
Clustering ist mit 33 \% (6/18) vertreten. Insgesamt zeigt sich, dass insbesondere \gls{nn} die dominierende Methodenklasse bilden.
In der Literatur dominieren \gls{dl}-Verfahren, insbesondere aufgrund ihrer Eignung zur Verarbeitung unstrukturierter Daten wie Logs, Metriken und Text.
