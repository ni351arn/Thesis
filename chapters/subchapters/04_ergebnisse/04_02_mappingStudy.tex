...
Die \gls{ci}/\gls{cd}- \& Pipeline-Optimierung zeigt insgesamt die niedrigsten Werte, bleibt aber über alle Herausforderungen hinweg relativ konstant ($n=9, 7, 9, 8, 8$). 
...
Im \gls{sl} dominiert die Kombination aus \gls{dl} \& \gls{nn} in allen Anwendungsfeldern. 
...
Auch die Optimierung von \gls{ci}/\gls{cd}-Pipelines wird häufig mit \gls{dl} adressiert (n=7). 
Insgesamt zeigt sich damit, dass \gls{sl} in der betrachteten Literatur vor allem in Verbindung mit \gls{dl} und neuronalen Netzen eingesetzt wird.

Neben \gls{dl} werden im \gls{sl} auch Ensemble- und baum-basierte Methoden sowie klassische Klassifikations- und Regressionsverfahren erwähnt, jedoch mit deutlich geringeren Häufigkeiten. 
...
Clustering wird im Kontext von \gls{sl} nicht beobachtet, sodass diese Zuordnungen nicht belegt sind (n=0).

Für \gls{ul} zeigt sich ein anderes Muster. Auch hier wird die Kombination aus \gls{dl} \& \gls{nn} über alle Anwendungsfelder hinweg am häufigsten genannt (n=6–8). 
Clustering-Verfahren treten als zweite zentrale Kategorie auf, werden jedoch deutlich seltener thematisiert (n=3–5).
Insgesamt deutet dies darauf hin, dass \gls{ul} hauptsächlich dort eingesetzt wird, wo Abweichungen und Muster ohne gelabelte Daten erkannt werden sollen (z.B. bei Anomalien oder Wartungsvorhersagen).
Andere Algorithmusklassen (z.B. Ensemble- bzw. klassische Klassifikations-/Regressionsverfahren) sind im \gls{ul}-Kontext nicht belegt (n=0), wodurch sich eine klare Fokussierung auf \gls{dl}/\gls{nn} und Clustering ergibt.

Im \gls{rl} konzentrieren sich die Zuordnungen ausschließlich auf \gls{dl} und \gls{nn}. 
Die Häufigkeiten sind insgesamt geringer als im \gls{sl}- und \gls{ul}. 
Die höchsten Werte zeigen sich bei der Sicherheits- und Bedrohungserkennung (n=7) sowie bei der Ressourcen- und Workload-Optimierung (n=6). Für die Optimierung von \gls{ci}/\gls{cd}-Pipelines und Self-Healing/Predictive Maintenance liegen die Werte jeweils bei n=5, während Intelligente Bereitstellung am seltensten genannt wird (n=4).
Andere Algorithmusklassen sind im \gls{rl}-Kontext nicht belegt (n=0), was die enge Kopplung von \gls{rl} an \gls{dl}-basierte (Deep-RL) Ansätze in den betrachteten Publikationen unterstreicht.
\end{comment}
