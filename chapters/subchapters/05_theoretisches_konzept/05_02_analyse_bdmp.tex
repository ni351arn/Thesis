...
Die \gls{bmlp} ist eine modular aufgebaute, Cloud-Native-Plattform zur Unterstützung von Fertigungs- und Logistikprozessen.
...
Fachliche Funktionen in der \gls{bmlp} werden als eigenständige Module in einer verteilten und modularen Grundstruktur umgesetzt.
...
Jedes Fertigungswerk kann je nach Anforderungen ein Cluster im lokalen Rechenzentrum für latenzkritische Anwendungen betreiben.
Für nicht latenzkritische Anwendungen werden zentrale oder regionale Rechenzentren genutzt.
Daraus ergibt sich ein hybrides Betriebsmodell, dessen Ausprägung von den jeweiligen Werksanforderungen abhängt.
...
Die Bereitstellung erfolgt nach dem GitOps-Ansatz.
...
Die Kommunikation zwischen Modulen erfolgt über öffentliche APIs, während interne Modulkommunikation ereignisbasiert umgesetzt ist.
...
Für \gls{aiops} sind nicht die Werkzeuge entscheidend, sondern die Daten, die im Plattformbetrieb entstehen.
In der \gls{bmlp} werden Betriebsdaten über Logs, Metriken, Traces sowie technische und fachliche Ereignisse erfasst und zentral nutzbar gemacht.
...
Auf dieser Grundlage kommen im Betrieb bereits erste unterstützende KI-Funktionen zum Einsatz.
Im IT-Service-Management werden Runbooks und dokumentierte Fehlerbilder über eine lokale KI (RAG)\footnote{Retrieval-Augmented Generation (RAG) kombiniert Sprachmodelle mit einer gezielten Abfrage externer Datenquellen, um domänenspezifische Informationen in die Textgenerierung einzubeziehen. Dadurch lassen sich fachliche Antworten präziser und nachvollziehbarer erzeugen als mit isolierten Sprachmodellen allein \cite{ARSLAN20243781}.} genutzt, um Störungen initial einzuordnen.
