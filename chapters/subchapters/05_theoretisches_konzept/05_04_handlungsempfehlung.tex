...
Gleichzeitig ist der erforderliche Aufwand überschaubar.
Die notwendigen Datenquellen sind bereits vorhanden, und es sind keine grundlegenden Änderungen an bestehenden Abläufen erforderlich.
...
Auf Basis der priorisierten Einordnung bietet sich der Einsatz von K8sGPT\footnote{\url{https://k8sgpt.ai}} als beispielhafte Umsetzung des identifizierten Use Cases an \cite{kankanalaAIMLDevOps2024}. 
In der Literatur wird K8sGPT als Open-Source-Projekt beschrieben, das den Einsatz von \gls{ki} zur Unterstützung des Betriebs containerbasierter Systeme adressiert.
...
Abbildung \ref{fig:k8sgptEinordnung} zeigt eine einfache Architekturskizze, die die Einordnung von K8sGPT im Plattformbetrieb darstellt.
\begin{figure}[H]
\centering
\input{figures/05_bewertung/05_04_k8sgpt.tex}
\caption{Einordnung von K8sGPT im Plattformbetrieb}
\label{fig:k8sgptEinordnung}
\end{figure}
Fehler entstehen entweder in der \gls{ci}/\gls{cd}-Pipeline oder in der Laufzeitumgebung des Kubernetes-Clusters.
Die dabei anfallenden Logs, Ereignisse und Zustände werden zentral gesammelt und bilden die Datenbasis für die Analyse.

K8sGPT wertet diese Daten aus und generiert darauf basierende Hinweise für die zuständigen Teams.
Das Werkzeug ist nicht in den Auslieferungsprozess eingebunden und hat keinen Einfluss auf Build oder Bereitstellung.
Die Verantwortung für Entscheidungen und Maßnahmen verbleibt vollständig bei den beteiligten Teams.
